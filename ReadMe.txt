...
2019.4.26
	...
	脚本执行将|，&，^优先级调为最低
	所有上位机消息加入集中器SN码字段
	使用c语言的随机数，每条发送消息都加一个cmdNum字段
	脚本运行：解决在声明过一次长字符串，下次声明短字符串还会带着长字符串的后几个字符的bug
2019.4.27
	重写dbg逻辑，使其更易于移植，在于上位机的通信中加入命令解析器调用
	去除空闲任务的唤醒机制
2019.4.28
	重写网络相关配置存储
	解决V2.3版键盘环绕灯指示异常的bug
	加入删除任务函数
2019.4.30
	解决按键彩灯每隔一段时间会异常闪烁的bug
	加入按键反馈
2019.5.5
	空闲任务进入主循环之前读取SD卡中的系统初始化脚本并执行
2019.5.6
	加入大于小于判断，解决不等号判断结果异常的bug
	新增一些注释，做了一些程序结构的修改
2019.5.8
	设备SN码后面增加设备ID
2019.5.11
	解决自动重启之后屏幕不亮的BUG
2019.5.13
	串口1在连续20次收到0x7f之后复位到ISP进行程序升级
2019.5.14
	RTC初始化失败时跳出初始化。
2019.5.15
	配置模式命令校验改为接受地址为0的和地址为本机的命令
	命令0x01返回的地址是自身地址
2019.5.23
	成功用集中器读取目标设备的IDCODE
	成功用集中器下载程序到目标板的SRAM
...
2019.7.9
	完善网口网络操作流程，新增读取无法送达地址的函数，
	解决调用dbg_print函数重启的bug
2019.7.10
	debug调试单独使用一个线程
	RTC加长等待时间，保证时钟初始化成功，在时钟初始化失败的情况下设置时间会返回错误
2019.7.18
	解决函数调用只有前4个参数有效的bug
	解决无法通过网络升级的bug
2019.7.19
	由于使用了多个任务操作W5500，W5500多个硬件驱动操作添加不可调度宏
	优化不可调度宏退出时的调度操作，只有退出最后一层时进行任务调度
	在不可调度期间调用了会造成任务调度的系统函数时忽略调度操作并返回调度失败
	delay_ms()和sleep_ms()可以在任务调度开始之前使用
	整理任务相关代码，编写详细的注释
	与服务器连接的端口号使用配置的端口号，并在初始化时判断端口号是否合理
	集中器运行时间显示格式改为天时分秒
	TCP发送数据时判断连接是否断开，保证一定是在连接状态下才发送数据
	在连接上服务器10秒之后发送上线消息，发送上线消息时如果发现连接断开就立即返回
2019.7.21
	解决函数调用时传入临时字符串死机的bug，在函数调用的时候可以传入任意个临时字符串参数，并且字符串长度没有限制
	解决findPair()函数返回的数据长度始终是从第0位开始的bug
	解决函数调用时双引号中有逗号时解析出错的bug
	添加播放指定旋律的功能，
	重写蜂鸣器驱动，减少蜂鸣器中断中的计算量
	解决设置无线模块信道函数数组越界造成的无限重启bug
2019.7.22
	run_beep()函数打开旋律文件播放成功，最大支持2kb的文件
	输入 fun run_beep("0:/beep.txt") 播放SD卡中的旋律文件
2019.7.23
	在系统开始运行之前也可以调用有临界区的函数
	调试任务在main函数里面创建
	可以使用串口输入调试命令，PD2引脚为RX，PC10为TX，串口调试和UDP端口调试不能同时使用
2019.7.31
	添加查询任务上下文的系统函数
	debug加入查询任务上下文命令
2019.8.2
	实现灯带呼吸灯效果
	解决软件定时器回调函数不回调的bug
	环境值正常时灯带绿色闪烁，环境值异常时灯带红色常亮
	无线查询到设备状态时更新集中器显示
2019.8.5
	重写蜂鸣器播放驱动，使用两个定时器，一个计算时长，另一个调制输出频率
	降低CPU使用率，提高PWM调制精度
	ping操作可以直接ping 网址
	ping操作统计的延时精度提高到1ms
	字符串转数字函数在字符有非数字字符时转化值为0
	使用定时器5，提供1ms的定时器中断
